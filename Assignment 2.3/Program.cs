...
namespace Assignment_2
{
    class Program
    {
        ...
        private int[][] HighFive(int[][] items)
        {
            try
            {
                var dict = new SortedDictionary<int, List<int>>();//seperate the studentID and score to store in the dictionary

                foreach (var student in items)
                {
                    var studentId = student[0];//store studentID
                    var score = student[1];//store student score
                    if (dict.ContainsKey(studentId))
                    {
                        dict[studentId].Add(score);//add score
                    }
                    else
                    {
                ...
                foreach (var student in dict)
                {
                    ...
                    ans[i][0] = student.Key;
                    var sum = 0;
                    //sort the score and get the high 5
